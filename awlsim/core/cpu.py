...
class S7Prog(object):
	...
	# What is DB, FB, FC, OB and UDT ?
	# DB - AWL Data Block
	# FB - AWL Function Block
	# FC - AWL Function
	# OB - AWL Organization Block (refer core/parser.py)
	...
	# -- destroy source ref for all blocks (DBs, FBs, FCs, OBs, and UDTs)
	# -- clear symbol table
	# -- reset all pending block lists to None
	def reset(self):
	...
	# -- merge arggument symbol table with existing program symbol table
	def loadSymbolTable(self, symbolTable):
		self.symbolTable.merge(symbolTable)


	# -- yield is an iterator which is returned
	# -- mnemonics are usually instruction names like MOV,ADD
	# -- Here it refers to whether the instructions are in english or german.
	# -- figures the language of the instructions and sets it to the cpu specs
	...
	# -- add some library blocks to self.cpu.fcs list or self.cpu.fbs list. reset self.pendingLibSelections
	# -- gets Library class for each lib selection from a registered set.
	# -- Instantiates a library block using the extracted Library class
	# -- translate block instructions 
	# -- add each translated block to cpu.fcs or cpu.fbs
	def __loadLibraries(self):
		for libSelection in self.pendingLibSelections:
			...
			if libEntryCls.isFC:
				...
				block = translator.translateLibraryCodeBlock(block) # --refer translator.py
	...
	# check instruction parameters for datatype compatibility for all call instructions (I think) in the passed block
		...
		for insn, calledCodeBlock, calledDataBlock in self.cpu.allCallInsns(block):
			try:
				for param in insn.params:
					...
					translator.translateParamAssignOper(param) # -- translate string to chars and implicit pointers. refer translator.py
			except AwlSimError as e:
				e.setInsn(insn)
				raise e
	...
	# Run static error checks for code block for all instructions in block
	def __staticSanityChecks_block(self, block):
		for insn in block.insns:
			insn.staticSanityChecks()

	# Run static error checks for all USer Code blocks
	def __staticSanityChecks(self):
		for block in self.cpu.allUserCodeBlocks():
			self.__staticSanityChecks_block(block)

	# -- iterates through each pendingUDTs, pendingOBs, pendingFBs and pendingDBs
	# -- uses resolver to extract udtnumber , OBnumber, FB number ... of respective entities
	# -- creates udt, OB, FB and FC objects from each number
	# -- adds the objects to self.cpu.udts, self.cpu.obs, self.cpu.fbs .. dictionaries with the respective numbers as keys.
	def build(self):
		translator = AwlTranslator(self.cpu)
		resolver = AwlSymResolver(self.cpu)
		...
		for rawOB in self.pendingRawOBs:
			...
			ob = translator.translateCodeBlock(rawOB, OB) # --refer translator.py
			if obNumber in self.cpu.obs:
				self.cpu.obs[obNumber].destroySourceRef()
			obs[obNumber] = ob
			self.cpu.obs[obNumber] = ob #-- add to cpu list
		...
		for rawFB in self.pendingRawFBs:
			...
			fb = translator.translateCodeBlock(rawFB, FB) # --refer translator.py
			if fbNumber in self.cpu.fbs:
				self.cpu.fbs[fbNumber].destroySourceRef()
			fbs[fbNumber] = fb
			self.cpu.fbs[fbNumber] = fb # --add to cpu list
		self.pendingRawFBs = []
		...
		for rawFC in self.pendingRawFCs:
			...
			fc = translator.translateCodeBlock(rawFC, FC) # --refer translator.py
			if fcNumber in self.cpu.fcs:
				self.cpu.fcs[fcNumber].destroySourceRef()
			fcs[fcNumber] = fc
			self.cpu.fcs[fcNumber] = fc # --add to cpu list
		self.pendingRawFCs = []


		# ??? - SFB and SFC are some system code blocks
		if not self.cpu.sfbs:
			...
			for sfbNumber in SFB_table.keys():
				...
				self.cpu.sfbs[sfbNumber] = sfb # --add to cpu list
		# ???
		if not self.cpu.sfcs:
			...
			for sfcNumber in SFC_table.keys():
				...
				self.cpu.sfcs[sfcNumber] = sfc # -- add to cpu list

		# Build the data structures of code blocks.
		for block in self.cpu.allCodeBlocks():
			block.interface.buildDataStructure(self.cpu) # -- refer blocks.py. adds each obj in IN, OUT, INOUT and STAT lists of the block into an awl struct.
		...
		for rawDB in self.pendingRawDBs:
			...
			db = translator.translateDB(rawDB) # -- refer translator.py
...
class S7CPU(object): #+cdef
	...
	# Returns all code blocks (OBs, FBs, FCs, SFBs, SFCs)
	def allCodeBlocks(self):
		for block in self.allUserCodeBlocks():
			yield block
		for block in self.allSystemCodeBlocks():
			yield block
	# -- return all call instructions, associated data blocks and code blocks in each call instruction operands
	# -- first extracts dataBlockOperators, then the operator's index, uses that index to extract the dataBlock using the cpu's dbs list.
	# -- similar approach for extracting the codeblock for each instruction. Uses the code block type to extract the code block from the correct cpu list : fcs, fbs, sfcs, sfbs ..
	...
	# ??? Need to look at prog.
	# -- sets the DB, FB, FC, OB and UDT lists of the program
	# -- starts program build - (see description above) if it needs to be rebuild
	...
	# -- reallocate 4 accumulators, specified number of Timers, Counters, Inputs and Outputs and Flags according to specifications
	def reallocate(self, force=False):
		...
		CallStackElem.resetCache() # -- reset Call stack

	# -- reset program, reallocate resources
	def reset(self):
		self.prog.reset() # --reset program
		...
		self.dbRegister = self.dbs[0] # --- DB[0] is initialized in this function just a few lines above. It is a read/write-protected system-DB
	...
	# -- set dbregister and diregister to dbs[0] - Why ?
	# -- resect accumulators 1-4, ar1 - ar2 and statusWord
	# -- set top of call stack to this block
	# -- iterate through each instruction in block
	# -- call instruction.run()
	# -- update instruction pointer accordingly
	# -- after block is finished, call BlockExit callback if defined
	# -- pop callStack to remove the curr block
	# -- call handleBlockExit
	...
	# Run startup code
	# -- run_OB(obs[102]) or run_OB(obs[100])
	def startup(self):
	...
	# -- run_OB(obs[1]).
	# -- call cycleExit callback if any
	# -- update some statistics
	def runCycle(self):
		# Run the actual OB1 code
		self.__runOB(self.obs[1]) # -- I think OB1 refers to user code
	...
	# -- raises an exception if runTimelimit is exceeded
	...
	# -- get offset for the given label
	...
	# -- extracts the function from the fcs list using the blockOper.
	# -- returns a CallStackElem created using the function
	...
	# -- extracts the FunctionBlock and DataBlock from respective lists using blockOper
	# -- returns a CallStackElem using the extracted FB and DBs and passed parameters
	# --reassigns values of dbRegister and Diregister, DBREGISTER points to Diregister, and Diregister is assgned the value of the DB. - Why ?
	...
	# -- calls the appropriate call function based on the blockOper type
	# -- returns a CallStackElem
	...
	# -- Call the correct call method based on Block oper type. Set it to the top of the call stack.
	# -- gets the returned CallStackElem and sets it to the Top of the Call Stack
	...
	# ???
	# -- I think it is aome branch instruction which simply changes the Jump offset of the current top CallStackElem
	# -- sets some statusWord flags
	def run_BE(self):
		s = self.statusWord
	...
	# ???
	# -- gets the correct DB by using the dbOper. Sets the value of dbregister or diregister accordingly
	...
	# -- swaps dbregister and diregister
	def run_TDB(self):
	...
	# returns the operator's value based on its type
...
#@cy	cpdef object fetch(self, object operator, set enforceWidth=set()):
	...
	def fetchE(self, operator, enforceWidth):
		...
		return self.inputs.fetch(operator.value, operator.width) #-- See ByteArray implementation
